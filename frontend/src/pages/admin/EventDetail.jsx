...
const quickLinks = [
  { to: 'policy', title: '대기열 정책 설정', desc: '이벤트의 트래픽 제어 및 대기열 RPS 정책을 설정합니다.', icon: 'gear' },
  { to: 'monitoring', title: '실시간 모니터링', desc: '이벤트의 실시간 RPS, 에러율, 대기열 길이 등을 모니터링합니다.', icon: 'monitor' },
  { to: 'ai-recommendation', title: 'AI 추천', desc: 'AI가 최적의 대기열 운영 정책을 분석하고 추천합니다.', icon: 'bulb' },
]
...
export default function EventDetail() {
  ...
  if (error && !event) {
    return (
      <>
        <h2 className="h5 fw-semibold mb-1">이벤트 정보</h2>
        <p className="text-muted mb-4">이벤트 정보를 확인하고 대기열 정책 및 모니터링 메뉴로 이동할 수 있습니다.</p>
        ...
        <div className="card-body">
          <div className="d-flex justify-content-between align-items-start flex-wrap gap-2">
            ...
            <div className="d-flex align-items-center gap-2">
              <span className={`badge ${ev.status === 'OPEN' ? 'bg-success' : ev.status === 'CLOSED' ? 'bg-secondary' : 'bg-warning text-dark'}`}>
                {ev.status}
              </span>
            </div>
          </div>
